...
from helpers.utils import SagaErrorPayload, failure_task_name, success_task_name


def message_to_channel(message: Message, response: Message = None,
                       publish_made_first=False, description: str = None) -> Tuple[str, Channel]:
    ...
    if response:
        channel_kwargs[second_action] = Operation(
            message=response,
        )
